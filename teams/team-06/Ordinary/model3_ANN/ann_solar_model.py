...
# CRITICAL FIX: Keep only rows where Status = 'Normal'
# This is the same filter applied in Decision Tree and Regression models.
# 'Waiting' rows have Pac(W) = 0 and do not represent real generation behavior.
# Without this filter, the ANN would train on artificial zeros, learning
# a fundamentally different pattern than the other models, making comparison unfair.
data = data[data["Status"] == "Normal"].reset_index(drop=True)
 
print("\n=== DATASET PREVIEW ===")
preview_cols = ['Day_year ', 'Pac(W)', 'INVTemp(\u2103)', 'AMTemp1(\u2103)', 'Hora_SIN', 'HORA_COS']
print(data[[c for c in preview_cols if c in data.columns]].head())
...
# Features -> same 23 variables used in Decision Tree and Regression models
# This ensures all models are evaluated under the same conditions (fair comparison).
#
...
# Impute any missing values with column median (same strategy as other models)
imputer = SimpleImputer(strategy="median")
X = imputer.fit_transform(X_raw)
 
print("\n=== VARIABLE SELECTION ===")
print(f"Target variable : Pac(W) — AC power output (Watts)")
...
# ============================================================
# ANNs are sensitive to feature scale. StandardScaler normalizes each feature
# to mean=0, std=1, which helps gradient-based solvers converge faster.
# The scaler is fitted ONLY on training data to prevent data leakage.
 
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)   # Fit on train, transform train
X_test_scaled  = scaler.transform(X_test)         # Transform test only (NO re-fitting)
...
def train_and_evaluate_ann(hidden_layer_sizes, activation, solver, max_iter):
    """
    Trains one ANN configuration and returns evaluation metrics + predictions.
    """
    model = MLPRegressor(
...
print(f"   The Baseline (7,) tanh + lbfgs achieves the best balance of accuracy and simplicity.")
...
print(f"   partly because max_iter=10 was insufficient for convergence. Experiment 3 (9,2)")
print(f"   also underperformed despite having more neurons, due to the sgd solver's sensitivity.")
 
print(f"\n3. Did adding more hidden layers always improve performance?")
print(f"   No. A single hidden layer (Baseline) outperformed two-layer architectures in this dataset.")
print(f"   Solar power output has a relatively direct relationship with electrical variables;")
print(f"   deep networks introduce unnecessary complexity without improving generalization.")
 
print(f"\n4. Which activation function worked best?")
print(f"   tanh worked best in this dataset. It maps inputs to (-1, 1), handles")
print(f"   standardized data effectively, and avoids the dying neuron problem of relu.")
 
print(f"\n5. Which solver worked best?")
print(f"   lbfgs worked best. It is a quasi-Newton optimization method that works")
print(f"   efficiently on small-to-medium datasets, converging faster than adam or sgd.")
 
print(f"\n6. How did max_iter affect results?")
print(f"   Experiment 2 used max_iter=10, which was far too low for convergence, resulting in")
print(f"   poor predictions. The Baseline used max_iter=438, allowing full convergence.")
print(f"   Experiment 3 used max_iter=765, but the sgd solver and logistic activation")
print(f"   were not well-suited to this dataset, so high iterations did not help.")
 
print(f"\n7. Which single model would you keep and why?")
print(f"   The Baseline (7,) tanh + lbfgs. It achieves the highest R² and lowest errors")
print(f"   with the simplest architecture (single layer, 7 neurons). It generalizes well,")
print(f"   trains fast, and is the most interpretable ANN configuration in this comparison.")