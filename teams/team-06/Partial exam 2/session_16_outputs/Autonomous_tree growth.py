...
FEATURE_COLS = ["actual_price", "discounted_price", "discount_percentage"]
...
def main() -> None:
    ...
    # --- START OF DATA CLEANING FOR TEAM 06 ---
    # We clean these columns so the rest of the professor's code doesn't crash
    ...
    if 'rating' in df.columns:
        # We use pd.to_numeric with errors='coerce' to handle any weird characters in ratings
...
# This is expected as deeper trees fit the training data more closely.

# 2. Which tree has the lowest validation/test error?
# Answer: The tree with max_depth=13 (validation_error=0.3437). 
# This model provides the most accurate predictions for unseen data.

# 3. At what depth does validation performance stop improving?
# Answer: At depth 13. When moving to depth 14, the validation error 
# increases to 0.3464, indicating the model starts to lose accuracy.

# 4. Does your dataset show overfitting as tree depth increases?
# Answer: Yes. While the training error continues to decrease (down to 0.1293), 
# the validation error starts to rise after depth 13. This "divergence" is 
# the classic sign of overfitting.

# 5. Which tree would you choose for deployment, and why?
# Answer: I would choose the tree with depth 13. It is the "Sweet Spot" 
# that minimizes validation error, meaning it will perform best when 
# predicting actual user satisfaction (ratings).

# 6. Explain the trade-off between model complexity and performance.
# Answer: Increasing complexity (depth) helps the model learn specific 
# details of the Amazon dataset, but too much complexity makes it 
# sensitive to "noise." For our streaming project, depth 13 captures 
# the general patterns of user retention without over-specializing..

# FINAL INTERPRETATION:
# To prevent users from canceling their streaming service, we need to 
# understand their ratings. A balanced model (Depth 13) allows us to 
# identify the real drivers of satisfaction, helping us create 
# effective retention strategies based on actual user data..
# =============================================================================