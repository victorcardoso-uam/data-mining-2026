...
DATA_PATH = "/Users/valeriagarcia/data-mining-2026/teams/team-05/session_16/diabetes_cleaned.csv"
...
def main() -> None:
    ...
    print(
        "6. Explain the trade-off between model complexity and "
        "performance in your dataset."
    )
    print()
...
# =============================================================================
# TEAM-05 INTERPRETATION — DIABETES PREDICTION DATASET
# =============================================================================

# 1. Which tree has the lowest training error?
# The Deep Tree (max_depth=10) with train_error=0.0284. Training error
# decreases continuously as depth increases because deeper trees fit
# the training data better.

# 2. Which tree has the lowest validation/test error?
# Both Shallow Tree and Best Tree (max_depth=2) with test_error=0.02895.
# These achieve the lowest generalization error on unseen data.

# 3. At what depth does validation performance stop improving?
# At depth=2. Validation error remains constant from depth=2 to depth=8
# (0.0289), then starts increasing from depth=9 onwards, indicating
# overfitting begins there.

# 4. Does your dataset show overfitting as tree depth increases?
# Yes, clear overfitting pattern observed:
# - Depths 1-8: Training error > Validation error (underfitting to fit)
# - Depths 9-15: Training error < Validation error (classic overfitting)
# - Gap widens: At depth=15, train_error=0.0223 but
#   validation_error=0.0336

# 5. Which tree would you choose for deployment, and why?
# max_depth=2 (Best/Shallow Tree). Reasons:
# - Lowest test error (0.02895)
# - Minimal complexity: only 3 leaves vs 75 leaves at depth=10
# - Highly interpretable for medical professionals
# - Reduces risk of learning noise from training data
# - Best bias-variance trade-off for diabetes prediction

# 6. Explain the trade-off between model complexity and performance.
# The diabetes dataset shows strong signal in just the first 2 splits.
# Beyond that:
# - Complexity grows EXPONENTIALLY: 3 → 656 leaves (depth 2 to 15)
# - NO performance gain: validation error plateaus and then degrades
# - This illustrates the bias-variance trade-off: simple models
#   generalize better
# - In medical prediction tasks, interpretability is also critical for
#   clinical use
